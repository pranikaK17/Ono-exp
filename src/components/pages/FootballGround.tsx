...
const liveEvent: Event = {
  ...
  date: 'Mar 9, 2026',
  time: 'LIVE NOW',
  venue: 'Football Ground — Main Pitch',
  description: 'The semi-finals are here. Watch the best college teams battle for a spot in the grand finale.',
  image: '',
}
...
const upcomingEvents: Event[] = [
  {
    ...
    date: 'Mar 15, 2026',
    time: '4:00 PM',
    venue: 'Football Ground — Goal Zone',
    description: 'Test your precision. 5 shots, 5 chances — can you beat the keeper?',
    image: '',
  },
  {
    ...
    time: '5:30 PM',
    venue: 'Football Ground — Main Pitch',
    description: 'The grand finale of the inter-college football tournament. May the best team win.',
    image: '',
  },
  {
    title: 'Alumni vs Students Match',
    date: 'Mar 18, 2026',
    time: '3:00 PM',
    venue: 'Football Ground — Main Pitch',
    description: 'Legends return to the field for a friendly showdown against the current squad.',
    image: '',
  },
]
...
const pastEvents: Event[] = [
  {
    ...
    date: 'Feb 28, 2026',
    time: '10:00 AM',
    venue: 'Football Ground — Centre Circle',
    description: 'The tournament kicked off with a ceremonial first touch and drone light show.',
    image: '',
  },
  {
    title: 'Quarter Finals — Day 1',
    date: 'Mar 5, 2026',
    time: '2:00 PM',
    venue: 'Football Ground — Main Pitch',
    description: 'Four intense matches decided who moves forward in the bracket.',
    image: '',
  },
]
    ...
    <div style={{
      ...
      <div style={{
        ...
        background: event.image
          ...
          : 'linear-gradient(135deg, rgba(34,197,94,0.1), rgba(77,216,230,0.08))',
        ...
        {badge && (
          <div style={{
            ...
            background: badge === '● LIVE' ? 'rgba(255,50,50,0.85)' : 'rgba(34,197,94,0.7)',
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ color: 'rgba(34,197,94,0.8)' }}>{event.date}</span>
          <span style={{ color: 'rgba(255,255,255,0.2)' }}>|</span>
          <span style={{ color: event.time === 'LIVE NOW' ? '#ff6b6b' : '#22c55e', fontWeight: 600 }}>{event.time}</span>
        </div>
        <div style={{
...
function SideCard({ event, onClick }: { event: Event; onClick?: () => void }) {
  return (
    ...
    >
      <div style={{
        height: 80,
        background: event.image ? `url(${event.image}) center/cover` : 'linear-gradient(135deg, rgba(34,197,94,0.08), rgba(77,216,230,0.06))',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
      ...
      <div style={{ padding: '10px 12px' }}>
        ...
        <div style={{ marginTop: 4, fontSize: '0.62rem', color: 'rgba(34,197,94,0.65)', fontFamily: "'Exo 2', sans-serif", letterSpacing: '0.05em', textTransform: 'uppercase' }}>
...
function SideColumn({ title, events, color }: { title: string; events: Event[]; color: string }) {
  return (
    <div style={{
      ...
      scrollbarWidth: 'thin', scrollbarColor: 'rgba(34,197,94,0.15) transparent',
    }}>
      <h3 style={{
...
const arrowBtnStyle: React.CSSProperties = {
  ...
  color: '#22c55e', fontSize: '1.4rem', fontWeight: 700,
  display: 'flex', alignItems: 'center', justifyContent: 'center',
...
export default function FootballGround({ onClose }: { onClose: () => void }) {
  ...
  const rightColor = tab === 'upcoming' ? '#ff6b6b' : '#22c55e'

  return (
    <div
      className={`page-overlay page-overlay-anim ${visible ? 'page-overlay-in' : 'page-overlay-out'}`}
    ...
    >
      <button className="page-overlay-close" onClick={handleClose}>✕</button>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: 10, marginTop: 16, flexShrink: 0 }}>
                {(tab === 'upcoming' ? upcomingEvents : pastEvents).map((_, i) => (
                  <button key={i} onClick={() => setSelectedIdx(i)} style={{
                    ...
                    background: i === selectedIdx ? '#22c55e' : 'rgba(255,255,255,0.15)',
