...
const liveEvent: Event = {
  ...
  date: 'Mar 9, 2026',
  time: 'LIVE NOW',
  venue: 'Cricket Ground — Main Pitch',
  description: 'The semi-final clash between the top college teams. Expect fierce bowling, big sixes, and a packed crowd.',
  image: '',
}

const upcomingEvents: Event[] = [
  {
    title: 'T10 Blitz Tournament',
    date: 'Mar 13, 2026',
    time: '9:00 AM',
    venue: 'Cricket Ground — Main Pitch',
    description: 'Fast-paced 10-over action with 8 teams battling it out in a single-day knockout format.',
    image: '',
  },
  {
    title: 'Bowling Speed Challenge',
    date: 'Mar 15, 2026',
    time: '4:00 PM',
    venue: 'Cricket Ground — Practice Nets',
    description: 'Test your arm! Radar-clocked bowling with prizes for the fastest delivery of the fest.',
    image: '',
  },
  {
    title: 'Cricket Quiz Night',
    date: 'Mar 17, 2026',
    time: '7:30 PM',
    venue: 'Cricket Ground — Pavilion',
    description: 'Think you know cricket? Teams of 3 compete in the ultimate cricket trivia showdown.',
    image: '',
  },
]
...
const pastEvents: Event[] = [
  {
    ...
    date: 'Feb 24, 2026',
    time: '10:00 AM',
    venue: 'Cricket Ground — Side Pitch',
    description: 'Six teams competed in a compact, high-energy box cricket format — crowds went wild.',
    image: '',
  },
  {
    ...
    time: '3:00 PM',
    venue: 'Cricket Ground — Main Pitch',
    description: 'Alumni legends returned for a friendly exhibition match against the current college XI.',
    image: '',
  },
]
    ...
    <div style={{
      ...
      <div style={{
        ...
        background: event.image
          ...
          : 'linear-gradient(135deg, rgba(234,179,8,0.1), rgba(251,146,60,0.08))',
        ...
        {badge && (
          <div style={{
            ...
            background: badge === '● LIVE' ? 'rgba(255,50,50,0.85)' : 'rgba(234,179,8,0.7)',
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ color: 'rgba(234,179,8,0.8)' }}>{event.date}</span>
          <span style={{ color: 'rgba(255,255,255,0.2)' }}>|</span>
          <span style={{ color: event.time === 'LIVE NOW' ? '#ff6b6b' : '#eab308', fontWeight: 600 }}>{event.time}</span>
        </div>
        <div style={{
        ...
        <div style={{
          ...
          <a href="#" style={{
            ...
            color: '#eab308', fontSize: '0.72rem', fontWeight: 600,
...
function SideCard({ event, onClick }: { event: Event; onClick?: () => void }) {
  return (
    <div onClick={onClick} style={{
      ...
      onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(234,179,8,0.35)'; e.currentTarget.style.transform = 'translateY(-2px)' }}
      onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(234,179,8,0.12)'; e.currentTarget.style.transform = 'translateY(0)' }}
    >
      <div style={{
        height: 80,
        background: event.image ? `url(${event.image}) center/cover` : 'linear-gradient(135deg, rgba(234,179,8,0.08), rgba(251,146,60,0.06))',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
      ...
      <div style={{ padding: '10px 12px' }}>
        ...
        <div style={{ marginTop: 4, fontSize: '0.62rem', color: 'rgba(234,179,8,0.65)', fontFamily: "'Exo 2', sans-serif", letterSpacing: '0.05em', textTransform: 'uppercase' }}>
...
function SideColumn({ title, events, color }: { title: string; events: Event[]; color: string }) {
  return (
    <div style={{
      ...
      scrollbarWidth: 'thin', scrollbarColor: 'rgba(234,179,8,0.15) transparent',
    }}>
      <h3 style={{
...
const tabStyle = (active: boolean): React.CSSProperties => ({
  ...
  border: active ? '1px solid rgba(234,179,8,0.5)' : '1px solid rgba(255,255,255,0.1)',
  background: active ? 'rgba(234,179,8,0.12)' : 'rgba(255,255,255,0.04)',
  color: active ? '#eab308' : 'rgba(255,255,255,0.45)',
...
const arrowBtnStyle: React.CSSProperties = {
  ...
  color: '#eab308', fontSize: '1.4rem', fontWeight: 700,
  display: 'flex', alignItems: 'center', justifyContent: 'center',
  backdropFilter: 'blur(8px)', transition: 'all 0.2s',
  boxShadow: '0 0 14px rgba(234,179,8,0.25)',
}

export default function CricketGround({ onClose }: { onClose: () => void }) {
  ...
  const rightColor = tab === 'upcoming' ? '#ff6b6b' : '#eab308'
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: 10, marginTop: 16, flexShrink: 0 }}>
                {(tab === 'upcoming' ? upcomingEvents : pastEvents).map((_, i) => (
                  <button key={i} onClick={() => setSelectedIdx(i)} style={{
                    ...
                    background: i === selectedIdx ? '#eab308' : 'rgba(255,255,255,0.15)',
