...
function createBokehLights(scene: THREE.Scene): { update(t: number): void; dispose(): void } {
  ...
  for (let i = 0; i < N; i++) {
    const angle = Math.random() * Math.PI * 2, r = Math.sqrt(Math.random()) * SPREAD_R
    positions[i * 3] = Math.cos(angle) * r; positions[i * 3 + 1] = HEIGHT_LO + Math.random() * (HEIGHT_HI - HEIGHT_LO); positions[i * 3 + 2] = Math.sin(angle) * r
    phases[i] = Math.random() * Math.PI * 2; speeds[i] = .08 + Math.random() * .16
    starFlag[i] = Math.random() < .35 ? 1 : 0; sizes[i] = starFlag[i] > .5 ? 14 + Math.random() * 22 : 130 + Math.random() * 200
  ...
  const mat = new THREE.ShaderMaterial({
    ...
    vertexShader: `uniform float time;attribute float phase,spd,sz,cIdx,isStar;varying float vAlpha,vCIdx,vStar;
      void main(){vec3 p=position;p.y+=sin(time*spd+phase)*5.5;p.x+=cos(time*spd*.35+phase*1.2)*2.5;p.z+=sin(time*spd*.28+phase*.9)*2.5;
      float tw=isStar>.5?pow(max(0.,sin(time*spd*4.+phase)),4.):.4+.6*(.5+.5*sin(time*spd*1.3+phase));
      vAlpha=tw;vCIdx=cIdx;vStar=isStar;vec4 mv=modelViewMatrix*vec4(p,1.);gl_Position=projectionMatrix*mv;gl_PointSize=sz*(160./-mv.z);}`,
    fragmentShader: `varying float vAlpha,vCIdx,vStar;
      vec3 nc(int ci){if(ci==0)return vec3(.20,.45,1.);if(ci==1)return vec3(.65,.15,1.);if(ci==2)return vec3(.00,.88,.82);if(ci==3)return vec3(1.,.25,.70);return vec3(.90,.95,1.);}
      void main(){vec2 uv=gl_PointCoord-.5;float d=length(uv);if(d>.5)discard;vec3 col=nc(int(vCIdx));float a;
      if(vStar>.5){float s=max(smoothstep(.06,.0,abs(uv.x))*smoothstep(.5,.0,abs(uv.y)),smoothstep(.06,.0,abs(uv.y))*smoothstep(.5,.0,abs(uv.x)));a=(1.-smoothstep(0.,.12,d))*.7+s*.9;}
      else{a=((1.-smoothstep(0.,.14,d))*.55+(1.-smoothstep(0.,.5,d))*.32)*vAlpha*.48;}gl_FragColor=vec4(col,a*vAlpha);}`,
...
function createBuildingOutline(scene: THREE.Scene, box: THREE.Box3, timeRef: { value: number }): () => void {
  const pad = 3, cx = (box.min.x + box.max.x) / 2, cz = (box.min.z + box.max.z) / 2, w = (box.max.x - box.min.x) + pad * 2, d = (box.max.z - box.min.z) + pad * 2
  ...
  const mat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `uniform float time;uniform vec2 sz;varying vec2 vUv;void main(){
      float lw=.048+.008*sin(time*1.6);float bx=min(vUv.x,1.-vUv.x),by=min(vUv.y,1.-vUv.y),bd=min(bx,by);if(bd>lw)discard;
      float arc;if(bx<by)arc=vUv.x<.5?vUv.y*sz.y:sz.x+sz.y+(1.-vUv.y)*sz.y;else arc=vUv.y<.5?sz.y+(1.-vUv.x)*sz.x:sz.y+sz.x+sz.y+vUv.x*sz.x;
      float dash=fract(arc*.08-time*1.6);if(dash<.30)discard;float pulse=.75+.25*sin(time*2.);
      float edge=smoothstep(lw,lw*.05,bd),glow=smoothstep(lw,0.,bd)*.5;
      vec3 col=mix(vec3(.5,1.,1.),vec3(1.,1.,1.),.6)*(2.2+pulse*.8);gl_FragColor=vec4(col,(edge+glow)*pulse);}`,
    transparent: true, depthWrite: false, blending: THREE.AdditiveBlending, side: THREE.DoubleSide,
  })
  const mesh = new THREE.Mesh(geo, mat); mesh.position.set(cx, .04, cz); mesh.renderOrder = 1; scene.add(mesh)
...
function createPinSparkles(scene: THREE.Scene, wp: THREE.Vector3): { update(t: number): void; dispose(): void } {
  const N = 40, ph = new Float32Array(N), sp = new Float32Array(N), r = new Float32Array(N), bh = new Float32Array(N), tw = new Float32Array(N), base = new Float32Array(N * 3)
  for (let i = 0; i < N; i++) { ph[i] = (i / N) * Math.PI * 2 + Math.random() * .4; sp[i] = .4 + Math.random() * .8; r[i] = .5 + Math.random() * 1.2; bh[i] = Math.random() * 3.2; tw[i] = Math.random() * Math.PI * 2; base[i * 3] = wp.x; base[i * 3 + 1] = wp.y; base[i * 3 + 2] = wp.z }
  const geo = new THREE.BufferGeometry()
  geo.setAttribute('position', new THREE.BufferAttribute(base, 3)); geo.setAttribute('phase', new THREE.BufferAttribute(ph, 1))
  geo.setAttribute('spd', new THREE.BufferAttribute(sp, 1)); geo.setAttribute('orb', new THREE.BufferAttribute(r, 1))
  geo.setAttribute('bh', new THREE.BufferAttribute(bh, 1)); geo.setAttribute('tw', new THREE.BufferAttribute(tw, 1))
  const mat = new THREE.ShaderMaterial({
    uniforms: { time: { value: 0 } },
    vertexShader: `uniform float time;attribute float phase,spd,orb,bh,tw;varying float vA;
      void main(){float t=time*spd+phase,dr=mod(time*.22+phase*.5,3.2),twk=pow(max(0.,sin(time*3.1+tw)),5.);vA=twk*(1.-dr/3.2)*.95+.04;
  ...
  for (const ring of RINGS) {
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      fragmentShader: `uniform float time,innerR,outerR;uniform vec2 centre;varying vec2 vWorld;
        ...
        float pulse=.70+.30*sin(time*1.8),alpha=fill*rim*dash*pulse*.95;vec3 col=vec3(.92,.97,1.)*(2.+pulse*.4);gl_FragColor=vec4(col,alpha);}`,
      transparent: true, depthWrite: false, blending: THREE.AdditiveBlending, side: THREE.DoubleSide,
    })
    const mesh = new THREE.Mesh(geo, mat); mesh.rotation.x = -Math.PI / 2; mesh.position.set(wp.x, .04, wp.z); mesh.renderOrder = 2
...
function createCharacterTrail(scene: THREE.Scene): { stamp(x: number, z: number): void; update(dt: number): void; dispose(): void } {
  const POOL = 300, BURST = 10, TRAIL_LIFE = 1.1, COLOURS = [0x00ffec, 0xff00cc, 0xaa44ff, 0x00cfff, 0xffffff, 0xff44aa]
  let colourIdx = 0
  const makeGeos = () => {
  ...
  let head = 0
  return {
    stamp(x, z) { for (let b = 0; b < BURST; b++) { const p = pool[head % POOL]; head++; const angle = Math.random() * Math.PI * 2, r = .1 + Math.random() * .7; p.age = 0; p.active = true; p.vx = Math.cos(angle) * r; p.vz = Math.sin(angle) * r; p.mesh.position.set(x + p.vx * .15, .06, z + p.vz * .15); p.mesh.rotation.y = Math.random() * Math.PI * 2; p.mesh.scale.setScalar(.5 + Math.random() * 1.1); p.mesh.visible = true; p.mat.color.setHex(COLOURS[colourIdx % COLOURS.length]); colourIdx++; p.mat.opacity = 1 } },
    update(dt) { for (const p of pool) { if (!p.active) continue; p.age += dt; const prog = p.age / TRAIL_LIFE; if (prog >= 1) { p.active = false; p.mesh.visible = false; continue }; p.mesh.position.x += p.vx * dt * 2.2; p.mesh.position.z += p.vz * dt * 2.2; p.mesh.rotation.y += dt * 1.5; p.mat.opacity = Math.pow(1 - prog, 1.4); p.mesh.scale.setScalar((.5 + (1 - prog) * 1.1) * (1 - prog * .5)) } },
    dispose() { for (const p of pool) { scene.remove(p.mesh); p.mesh.geometry.dispose(); p.mat.dispose() }; geoT.forEach(g => g.dispose()) },
  }
}
...
export default function Map({ onPinClick, activePage }: { onPinClick?: (page: string) => void; activePage?: string | null }) {
  ...
  useEffect(() => {
    ...
    const fill = new THREE.DirectionalLight(0xadd8ff, .55)
    ...
    let trailTimer = 0; const TRAIL_INTERVAL = 0.09; let lastTrailPos = new THREE.Vector3()
    const INTERACT_RADIUS = 12, AUTO_CLOSE_RADIUS = 20
    ...
    if (promptEl && isTouch) promptEl.innerHTML = 'Tap here to interact'
    ...
    const tick = () => {
      ...
      if (charCtrl) {
        trailTimer += dt
        if (trailTimer >= TRAIL_INTERVAL && charCtrl.position.distanceTo(lastTrailPos) > .08) {
          trail.stamp(charCtrl.position.x, charCtrl.position.z)
          lastTrailPos.copy(charCtrl.position); trailTimer = 0
        }
      }
      trail.update(dt)
      if (charCtrl && promptEl) {
        ...
        for (const pw of pinWorldPositions) {
          const dx = charCtrl.position.x - pw.pos.x, dz = charCtrl.position.z - pw.pos.z, dist = Math.sqrt(dx * dx + dz * dz)
          if (dist < INTERACT_RADIUS && dist < closestDist && PIN_TO_PAGE[pw.name]) { closest = pw.name; closestDist = dist }
        }
        nearbyPin = closest
        promptEl.style.opacity = nearbyPin ? '1' : '0'
        promptEl.style.transform = nearbyPin
          ? (isTouch ? 'translate(-50%,-50%) scale(1)' : 'translate(-50%,0) scale(1)')
          : (isTouch ? 'translate(-50%,calc(-50% + 8px)) scale(.95)' : 'translate(-50%,8px) scale(.95)')
      ...
      ; (async () => {
        try {
          ...
          mapGLTF.scene.updateMatrixWorld(true); scene.add(mapGLTF.scene); mapGLTF.scene.updateMatrixWorld(true)
          for (const [_, box] of Object.entries(bldBoxMap)) outlines.push(createBuildingOutline(scene, box, sharedTime))
      ...
      <style>{`
        *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
        ...
        #screentone::after{content:'';position:absolute;inset:0;
          background-image:repeating-linear-gradient(-45deg,transparent,transparent 3px,rgba(0,0,0,0.055) 3px,rgba(0,0,0,0.055) 4px);
